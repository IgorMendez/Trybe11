...
function Table() {
  ...
  const setData = ({ filterNumValues, comp, filterNumber }) => {
    const data2 = data.filter((e) => {
      if (comp === 'maior que' && parseInt(e[filterNumValues], 10) !== null) {
        return parseInt(e[filterNumValues], 10) > parseInt((filterNumber), 10);
      }
    ...
    submitSearch({ ...arrayFiltered, arr: data2 })
    // console.log(data2);
    // console.log(data);
    // console.log(filters);
  ...
  const compare = arrayFiltered.arr ? arrayFiltered.arr : data;
